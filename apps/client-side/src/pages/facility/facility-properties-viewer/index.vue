<!-- :row-class="rowClass" -->
<template>
	<DataTable ref="dataview" v-model:selection="currentProperty" v-model:filters="filters"
		class="p-datatable-sm fadein animation-duration-300 animation-iteration-1 animation-ease-in" csv-separator=";"
		:export-filename="`${transformedDate}-${currentFacility['name']}-Свойства`" scrollable filterDisplay="menu" scrollHeight="680px" sortField="id"
		:sort-order="-1" paginator :global-filter-fields="['name', 'description', 'is_visible', 'is_fast', 'id', 'value', 'unit']" :rows="15" resizable-columns
		column-resize-mode="fit" show-gridlines :value="properties" selection-mode="single" dataKey="id" :rows-per-page-options="[5, 10, 15, 20, 25, 30]"
		:meta-key-selection="false" tableStyle="min-width: 50rem"
		...
		<template #header>
			<div class="tw-flex tw-w-full tw-items-center tw-justify-between">
				<SplitButton v-if="authorities === 'ADMIN'" label="Обновить" outlined icon="pi pi-refresh" :model="propertyActions" size="small"
					@click="handleRefresh()" />
		...
		<div class="tw-flex tw-h-full tw-flex-col tw-justify-between">
			<div class="tw-mt-[calc(1.25rem_+_1.5px)] tw-flex tw-flex-col tw-space-y-4">
				<InputWithHelp help="Введите текст.">
					<InputText v-model="name" placeholder="Наименование свойства (или тип)." class="p-inputtext-sm" :disabled="isCreateLoading" />
				</InputWithHelp>
				<InputWithHelp help="Введите текст (опционально).">
					<InputText v-model="description" placeholder="Описание свойства" class="p-inputtext-sm" :disabled="isCreateLoading" />
				</InputWithHelp>
				<InputWithHelp help="Введите значение.">
					<InputText v-model="value" placeholder="Значение свойства" class="p-inputtext-sm" autofocus :disabled="isCreateLoading" />
				</InputWithHelp>
				<InputWithHelp help="Выберите еденицу измерения.">
					<Dropdown v-model="unit" class="p-inputtext-sm" :options="units" optionLabel="label" showClear placeholder="Единица измерения"
						:disabled="isCreateLoading" />
				</InputWithHelp>
				<InputWithHelp help="Серый - Дефолт, Оранжевый - Предупреждение, Красный - Опасность.">
					<Dropdown v-model="color" class="p-inputtext-sm" :options="colors" optionLabel="label" showClear placeholder="Дефолт"
						:disabled="isCreateLoading" />
				</InputWithHelp>
				<InputWithHelp help="Выберите опцию из списка.">
					<Dropdown v-model="currentOption" class="p-inputtext-sm" :options="options" optionLabel="name" showClear placeholder="Опция"
						:disabled="isCreateLoading">
	...
	<Sidebar v-model:visible="updateSidebarVisible">
		...
		<div class="tw-flex tw-h-full tw-flex-col tw-justify-between">
			<div class="tw-mt-[calc(1.25rem_+_1.5px)] tw-flex tw-flex-col tw-space-y-4">
				<InputWithHelp help="Введите текст.">
					<InputText v-model="updName" placeholder="Наименование свойства (или тип)." class="p-inputtext-sm" :disabled="isUpdateLoading" />
				</InputWithHelp>
				<InputWithHelp help="Введите текст (опционально).">
					<InputText v-model="updDescription" placeholder="Описание свойства" class="p-inputtext-sm" :disabled="isUpdateLoading" />
				</InputWithHelp>
				<InputWithHelp help="Введите значение.">
					<InputText v-model="updValue" placeholder="Значение свойства" class="p-inputtext-sm" autofocus :disabled="isUpdateLoading" />
				</InputWithHelp>
				<InputWithHelp help="Выберите еденицу измерения.">
					<Dropdown v-model="updUnit" class="p-inputtext-sm" :options="units" optionLabel="label" showClear placeholder="Единица измерения"
						:disabled="isUpdateLoading" />
				</InputWithHelp>
				<InputWithHelp help="Серый - Дефолт, Оранжевый - Предупреждение, Красный - Опасность.">
					<Dropdown v-model="updColor" class="p-inputtext-sm" :options="colors" optionLabel="label" showClear placeholder="Дефолт"
						:disabled="isUpdateLoading" />
				</InputWithHelp>
				<InputWithHelp help="Выберите опцию из списка.">
					<Dropdown v-model="currentOption" class="p-inputtext-sm" :options="options" optionLabel="name" showClear placeholder="Опция"
...
<script setup lang="ts">
	...
	const propertyActions = [
		...
		{ label: 'Редактировать', icon: 'pi pi-cog', disabled: true, command: () => (updateSidebarVisible.value = true) },
	...
	const handleDelete = () => store.dispatch(propertyModel.actions['deletePropertyAsync'], { id: currentProperty.value['id'] })
</script>
