			...
			<template #header>
				<div class="tw-flex tw-justify-between">
					...
					<DataViewLayoutOptions
						v-model="layout"
						class="fadein animation-duration-300 animation-iteration-1 animation-ease-in"
					/>
				</div>
			</template>
			<template #list="slot">
...
<script setup lang="ts">
	...
	const currentFacilities: ComputedRef<Facility[]> = computed(() =>
		!query.value
			? store.getters[facilityModel.getters['useList']]
			: store.getters[facilityModel.getters['useSearchedList']](query)
	)
